...
$defaultvalpos = 9;
$debug = 0;
if ($#ARGV > 0) {$valpos = shift @ARGV;} else {$valpos = $defaultvalpos;}
if ($#ARGV > 0) {$rel = shift @ARGV;} else {$rel = "-";}
$nonrel = ""; # set to "" to not worry about it
...
while(<>) {
    @_ = split;
    if ($header && $_[$valpos]) {$label = $_[$valpos]; undef $header;}
